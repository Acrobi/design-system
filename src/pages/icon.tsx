...
import { iconMetaphors, type IconMetaphor } from '../lib/icon-metaphors'

export default function IconTestPage() {
  const [selectedIcon, setSelectedIcon] = useState<IconMetaphor | null>(null)
  const [iconSize, setIconSize] = useState('4')
  const [searchTerm, setSearchTerm] = useState('')

  const filteredIcons = iconMetaphors.filter(icon =>
    ...
    <div className="container mx-auto p-8 space-y-8">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-12 gap-4 p-6 border rounded-lg">
          {filteredIcons.map((metaphor) => (
            <button
              key={metaphor}
              onClick={() => setSelectedIcon(metaphor)}
              className={`flex flex-col items-center justify-center p-3 rounded-md border transition-colors hover:bg-accent hover:text-accent-foreground ${
                selectedIcon === metaphor ? 'bg-primary text-primary-foreground' : 'bg-background'
              }`}
              title={metaphor}
            >
              <Icon
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-4">
            {['chevron-left', 'chevron-right', 'chevron-up', 'chevron-down', 'arrow-left', 'arrow-right', 'home', 'menu'].map(metaphor => (
              <div key={metaphor} className="flex items-center gap-2 p-2 border rounded">
                <Icon metaphor={metaphor as IconMetaphor} className="h-4 w-4" />
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-4">
            {['add', 'remove', 'delete', 'edit', 'save', 'close', 'copy', 'send'].map(metaphor => (
              <div key={metaphor} className="flex items-center gap-2 p-2 border rounded">
                <Icon metaphor={metaphor as IconMetaphor} className="h-4 w-4" />
                <span className="text-sm">{metaphor}</span>
              </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-4">
            {['success', 'warning', 'error', 'info', 'help'].map(metaphor => (
              <div key={metaphor} className="flex items-center gap-2 p-2 border rounded">
                <Icon metaphor={metaphor as IconMetaphor} className="h-4 w-4" />
                <span className="text-sm">{metaphor}</span>
              </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-4">
            {['user', 'users', 'settings', 'bell', 'search', 'upload', 'download', 'folder'].map(metaphor => (
              <div key={metaphor} className="flex items-center gap-2 p-2 border rounded">
                <Icon metaphor={metaphor as IconMetaphor} className="h-4 w-4" />
                <span className="text-sm">{metaphor}</span>
              </div>
