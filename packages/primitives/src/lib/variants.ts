...
// Icon size mappings
export const iconSizeMappings: Record<SizePrimitive, string> = {
  xs: 'h-3 w-3',
  sm: 'h-4 w-4',
  md: 'h-5 w-5',
  lg: 'h-6 w-6',
...
export const getSizeClass = (size: SizePrimitive, type: 'component' | 'icon' = 'component'): string => {
  if (type === 'icon') {
...
export const getResponsiveSizeClasses = (
  size: SizePrimitive,
  responsiveSizes?: Partial<Record<SizePrimitive, SizePrimitive>>
): string => {
...
// Export all variant systems and utilities
export {
  type SizePrimitive,
  type ButtonVariant,
  type LabelVariant,
