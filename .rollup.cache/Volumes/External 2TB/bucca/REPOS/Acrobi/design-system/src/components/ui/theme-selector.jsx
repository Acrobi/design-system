...
export function ThemeSelector() {
    ...
    return (<div className="flex items-center gap-3">
      ...
      <Button variant="ghost" size="sm" onClick={toggleMode} className="px-2 py-1 h-8" title={`Mode: ${mode} - Click to toggle`}>
      ...
      <div className="flex items-center gap-2">
        ...
        <select value={theme} onChange={(e) => {
          ...
          {colorThemes.map((themeOption) => (<option key={themeOption.name} value={themeOption.name}>
              {themeOption.label}
            </option>))}
        </select>

        {/* Theme preview button - cycles through themes */}
        <Button variant="outline" size="sm" onClick={toggleTheme} className="px-2 py-1 h-8 w-8 p-0" title={`Current: ${theme} - Click to cycle`}>
          {/* 🚨 DO NOT modify this preview display */}
          <div className={cn(((_a = colorThemes.find(t => t.name === theme)) === null || _a === void 0 ? void 0 : _a.preview) || "", "w-3 h-3 rounded-sm")}/>
        </Button>
      </div>
    </div>);
}
//# sourceMappingURL=theme-selector.jsx.map