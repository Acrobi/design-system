...
export function Asset({ metaphor, className, alt, width, height, fallback, onLoad, onError, }) {
    ...
    // Loading state
    if (isLoading) {
        return (<div className={cn('animate-pulse bg-muted rounded', 'flex items-center justify-center', className)} style={{ width, height }} role="img" aria-label={`Loading ${metaphor}`}>
    ...
    if (error || !hasAsset) {
        ...
        return (<div className={cn('bg-muted border border-dashed border-muted-foreground/25 rounded', 'flex items-center justify-center text-muted-foreground text-sm', className)} style={{ width, height }} role="img" aria-label={`Missing ${metaphor}`} title={error ? `Error: ${error}` : `No asset found for "${metaphor}"`}>
    ...
    if (type === 'svg' || filename.endsWith('.svg')) {
        return (<img src={url} alt={alt || metaphor} className={className} width={width} height={height} onLoad={onLoad} onError={onError} style={{ width, height }}/>);
    }
    // Image assets (png, jpg, webp, etc.)
    if (type === 'image' || /\.(png|jpe?g|webp|gif|avif)$/i.test(filename)) {
        return (<img src={url} alt={alt || metaphor} className={cn('object-contain', className)} width={width} height={height} onLoad={onLoad} onError={onError} style={{ width, height }}/>);
    }
    // Icon assets - try to render as img first, fallback to div
    if (type === 'icon') {
        return (<img src={url} alt={alt || metaphor} className={cn('inline-block', className)} width={width || 24} height={height || 24} onLoad={onLoad} onError={(e) => {
