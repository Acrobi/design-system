...
const TabsList = React.forwardRef((_a, ref) => {
    ...
    return (<TabsPrimitive.List ref={ref} className={cn("inline-flex h-10 items-center justify-center rounded-md bg-muted p-1 text-muted-foreground", orientation === "vertical" && "flex-col h-auto", className)} {...props}/>);
});
TabsList.displayName = TabsPrimitive.List.displayName;
const TabsTrigger = React.forwardRef((_a, ref) => {
    var { className, variant, size, orientation } = _a, props = __rest(_a, ["className", "variant", "size", "orientation"]);
    return (<TabsPrimitive.Trigger ref={ref} className={cn(tabsVariants({ variant, size, orientation }), className)} {...props}/>);
});
