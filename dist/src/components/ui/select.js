/**
 * @prop {variant} {enum} {default|destructive|outline|secondary|ghost} {default} - The visual style of the select.
 * @prop {size} {enum} {default|sm|lg} {default} - The size of the select.
 * @prop {disabled} {boolean} {false} - Whether the select is disabled.
 * @prop {required} {boolean} {false} - Whether the select is required.
 * @prop {error} {boolean} {false} - Whether the select has an error state.
 */
"use client";
...
const selectVariants = cva("flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50", {
    variants: {
        variant: {
            default: "border-input bg-background text-foreground",
            destructive: "border-destructive bg-background text-destructive-foreground",
        ...
        error: {
            true: "border-destructive bg-destructive/5 text-destructive-foreground focus:ring-destructive",
        }
    },
    defaultVariants: {
        variant: "default",
...
const SelectTrigger = React.forwardRef((_a, ref) => {
    var { className, variant, size, error, children } = _a, props = __rest(_a, ["className", "variant", "size", "error", "children"]);
    return (_jsxs(SelectPrimitive.Trigger, Object.assign({ ref: ref, className: cn(selectVariants({ variant, size, error }), className) }, props, { children: [children, _jsx(SelectPrimitive.Icon, { asChild: true, children: _jsx(ChevronDown, { className: "h-4 w-4 opacity-50" }) })] })));
});
